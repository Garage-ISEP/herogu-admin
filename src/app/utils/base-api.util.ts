...
export abstract class BaseApi {
  ...
  protected post<Q, R>(path: string, body?: Q, observe_captcha: string | boolean = false) {
    ...
    if (typeof observe_captcha === 'boolean' && observe_captcha)
      return this.http.post<R>(`${environment.api}/${path}`, body, { reportProgress: true, observe: 'events', headers }).pipe(finalize(() => this.progress.hide()));
    else
      return this.http.post<R>(`${environment.api}/${path}`, body, { headers }).toPromise().finally(
        () => this.progress.hide()
      );
  }
  ...
  protected async patch<Q, R>(path: string, body?: Q, captcha?: string) {
    ...
    try {
      if (path.startsWith("/"))
        path = path.substring(1);
      const headers = new HttpHeaders({ Authorization: `Bearer ${this.token}`, recaptcha: captcha || '' });
      return await this.http.patch<R>(`${environment.api}/${path}`, body, { headers }).toPromise();
    } finally {
      this.progress.hide();
    }
  }
  ...
  protected async delete<R>(path: string, captcha?: string) {
    ...
    try {
      if (path.startsWith("/"))
        path = path.substring(1);
      const headers = new HttpHeaders({ Authorization: `Bearer ${this.token}`, recaptcha: captcha || '' });
      return await this.http.delete<R>(`${environment.api}/${path}`, { headers }).toPromise();
    } finally {
      this.progress.hide();
    }
  }
  ...
  protected set token(val: string | null) {
    if (val)
      localStorage.setItem("admin-token", val);
    else
